...
const ALLOWED_WEB_ORIGIN = "https://kangeol.github.io";
const CORS_ALLOW_METHODS = "POST, OPTIONS";
const CORS_ALLOW_HEADERS = "Content-Type";

// 중요: doPost/doGet/doOptions는 반드시 Apps Script 파일의 최상위(Global Scope)에 있어야 합니다.
// 다른 함수 안에 넣으면 Web App 배포에서 doPost/doGet을 찾지 못합니다.
function doPost(e) {
  try {
...
function jsonResponse(data) {
  const payload = Object.assign({}, data, {
    cors: data && data.cors ? data.cors : getCorsPolicy(),
  });

  // Apps Script ContentService의 TextOutput은 setHeader API를 제공하지 않습니다.
  // 따라서 프론트는 CORS preflight가 발생하지 않는 text/plain simple request로 호출합니다.
  // 이 cors 필드는 배포/디버깅 시 허용 정책을 확인하기 위한 명시적 메타데이터입니다.
